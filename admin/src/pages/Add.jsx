...
const Add = ({ token }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(backendURL + "/api/products/add", data, {headers: { token }});
      if(res.data.success) {
  ...
  return (
    <form onSubmit={handleSubmit} className='flex flex-col w-full items-start gap-3'>
      ...
      <div className='w-full'>
        ...
        <input onChange={(e) => setName(e.target.value)} className='w-full max-w-[500px] px-3 py-2' type="text" value={name} placeholder='Enter Product Name' required />
      </div>
      ...
      <div className='flex flex-col sm:flex-row gap-2 w-full sm:gap-8'>
        ...
        <div>
          ...
          <input onChange={(e) => setPrice(e.target.value)} value={price} className='w-full sm:w-[120px] px-3 py-2' type="number" placeholder='Enter Price' />
        </div>
      </div>
